...
int pepa_one_direction_copy(pepa_fds_t *fdx, buf_t *buf){
	...
	do {
		...
		// slog_note_l("Going to read from %s fd %d to buf size %ld", fdx->name_read, fdx->fd_read, buf->room);
		...
		if (rx < 0) {
			slog_error_l("Could not read: from %s %s", fdx->name_read, strerror(errno));
			ret = -PEPA_ERR_BAD_SOCKET_READ;
			goto endit;
		}
		...
		// slog_note_l("Going to write to %s fd %d from buf size %d, used %d",
		fdx->name_write,
		fdx->fd_read,
		rx, rx);
...
do {
	if (fdx->fd_write_mutex) {
		sem_wait(fdx->fd_write_mutex);
			...
			if (tx_current < 0) {
				ret = -PEPA_ERR_BAD_SOCKET_WRITE;
		slog_warn_l("Could not write to %s: returned -1: %s", fdx->name_write, strerror(errno));
				goto endit;
			}
	...
	slog_note_l("%s -> %s written_acc %d, rd_acc = %d", fdx->name_read, fdx->name_write, written_acc, rd_acc);
	}
	#endif
	return ret;
}
...
static void pepa_one_direction_rw_thread_cleanup(void *arg){
	...
	slog_note_l("Running RW THREAD EXIT [%s] cleanup routine", fdx->my_name);
	...
	if (fdx->close_read_sock) {
		...
		if (0 != close_rc) {
			slog_error_l("%s: Could not close reading socket: %s", fdx->my_name, strerror(errno));
		}
	}
	...
	if (fdx->buf) {
		...
		if (BUFT_OK != rc) {
			slog_error_l("Could not release buf_t");
		}
	}
	...
	slog_debug_l("***** %-14s TRANSFER THREAD ENDED: TRANSFERED: reads: %-5lu, writes: %-5lurx bytes: %-10lu Kb:%-8lu tx: bytes: %-10lu Kb:%-8lu *****",
...
void *pepa_one_direction_rw_thread(void *arg){
	...
	if (rc < 0) {
		slog_error_l("%s:Could not init CTL", fdx->my_name);
		pthread_exit(NULL);
	}

	slog_note_l("%s: Starting: %s -> %s, fd read: %d, fd_write: %d",
		fdx->my_name, fdx->name_read, fdx->name_write, fdx->fd_read, fdx->fd_write);

	if (fdx->fd_read < 0 || fdx->fd_write < 0) {
		slog_error_l("%s: One of descriptors is invalid", fdx->my_name);
		pthread_exit(NULL);
	}
	...
	if (0 != epoll_ctl_add(epoll_fd, fdx->fd_read, EPOLLIN | EPOLLRDHUP | EPOLLHUP)) {
		slog_warn_l("%s: Tried to add fdx->fd_read = %d and failed", fdx->my_name, fdx->fd_read);
		pthread_exit(NULL);
	}
	...
	if (epoll_ctl_add(epoll_fd, fdx->fd_write, EPOLLOUT | EPOLLRDHUP | EPOLLHUP)) {
		...
		slog_warn_l("%s: Tried to add fdx->fd_write = %d and failed", fdx->my_name,  fdx->fd_write);
		pthread_exit(NULL);
	}
	...
	do {
		...
		if (event_count < 0) {
			slog_fatal_l("%s: error on wait: %s", fdx->my_name, strerror(err));
			pthread_exit(NULL);
		}
		...
		for (i = 0; i < event_count; i++) {
			...
			if (events[i].events & (EPOLLRDHUP | EPOLLHUP)) {
				slog_warn_l("%s: Connection |%s = %d| is closed, terminate thread: %s", fdx->my_name,
			...
			if ((events[i].data.fd == fdx->fd_read) && (events[i].events & EPOLLIN)) {
				...
				if (rc < 0) {
					slog_warn_l("%s: Read/Write op between sockets failure: fd read: %d. fd out: %d",
		...
		if (0 != pepa_test_fd(fdx->fd_read)) {
			slog_warn_l("%s: read fd is invalid, terminate", fdx->my_name);
			pthread_exit(NULL);
		}

		if (0 != pepa_test_fd(fdx->fd_write)) {
			slog_warn_l("%s: write fd is invalid, terminate", fdx->my_name);
			pthread_exit(NULL);
		}
	#endif

		if ((fdx->reads > 0) && (0 == (fdx->reads % RX_TX_PRINT_DIVIDER))) {
			slog_debug_l("===== %-14s TRANSFER THREAD WORKING: TRANSFERED: reads: %-7lu writes: %-7lurx bytes: %-10lu Kb:%-8lu tx: bytes: %-10lu Kb:%-8lu =====",
			   fdx->my_name, fdx->reads, fdx->writes, fdx->rx, fdx->rx / 1024, fdx->tx, fdx->tx / 1024);
		}
	} while (1);
...
{
	pepa_socket_shutdown_and_close(core->sockets.in_listen, "IN LISTEN");
...
int pepa_open_listening_socket(struct sockaddr_in *s_addr, const buf_t *ip_address, const int port, const int num_of_clients, const char *name)
{
	...
	slog_debug_l("Open Socket [from %s]: starting for %s:%d, num of clients: %d",
	...
	if (rc < 0) {
		slog_error_l("Open Socket [from %s]: Could not set SO_NOSIGPIPE on socket, error: %s", name, strerror(errno));
		return (-PEPA_ERR_SOCKET_CREATION);
	}
#endif
