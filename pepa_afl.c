...
// pepa_core_t *afl_fill_core_with_input(const char *input, size_t input_size);

pepa_core_t *afl_fill_core_with_input(const char *input,
...
int test_0(const char *input,
           __attribute__((unused)) const size_t input_size)
{
...
{
    int sock_num = *((int *)input);
    int rc = pepa_socket_shutdown_and_close(sock_num, NULL);
    if (rc) {
        slog_error_l("pepa_socket_shutdown_and_close returned %d", rc);
    }
    rc = pepa_socket_shutdown_and_close(sock_num, "TEST");
    if (rc) {
        slog_error_l("pepa_socket_shutdown_and_close returned %d", rc);
    }
    return 0;
