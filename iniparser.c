...
static void trim(const char *str, int *begin_ind, int *end_ind)
{
    int b = *begin_ind;
    int e = *end_ind;
    int i;
...
static int read_line(struct read_ini *read_ini)
{
    int i = 0;
    int new = 0;
    ...
    if (new) read_ini->tmp[i] = '\0';

    return new;
}

static struct section *parse_section(struct read_ini *read_ini)
{
    if (NULL == read_ini) {
        return NULL;
    }
    ...
    for (;;) {
        ...
        if (new_i) {
            ...
            /* check for comments */
            if (read_ini->tmp[b] == '#' || read_ini->tmp[b] == ';') continue;
...
void ini_pp(const struct ini *ini)
{
    ...
    for (s = 0; s < ini->num_sections; s++) {
        ...
        for (c = 0; c < ini->sections[s]->num_configs; c++) printf("  key: \"%s\", value: \"%s\"\n",
